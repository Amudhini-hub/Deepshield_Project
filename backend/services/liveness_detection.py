...
class LivenessDetector:
    ...
    def _load_models(self):
        ...
        try:
            ...
            logger.warning(f"Could not load liveness model: {e}. Using fallback detection.")
            self.liveness_model = None
        ...
        try:
            ...
            for frame in sample_frames:
                ...
                if processed is not None:
                    ...
                    liveness_prob = float(prediction[0][1])  # probability of being alive
    ...
    def _detect_rppg(self, frames: List[np.ndarray]) -> float:
        ...
        try:
            ...
            r_norm = (np.array(r_channel) - np.mean(r_channel)) / (np.std(r_channel) + 1e-8)
            g_norm = (np.array(g_channel) - np.mean(g_channel)) / (np.std(g_channel) + 1e-8)
            b_norm = (np.array(b_channel) - np.mean(b_channel)) / (np.std(b_channel) + 1e-8)
