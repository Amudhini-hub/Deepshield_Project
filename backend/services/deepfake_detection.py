    ...
    def detect_from_frames(self, frames: List[np.ndarray]) -> DeepfakeResult:
        ...
        return DeepfakeResult(
            ...
            details={
                ...
                "models_available": self.mesonet_model is not None
                or self.xception_model is not None,
            },
    ...
    def _frequency_analysis(self, frames: List[np.ndarray]) -> float:
        ...
        try:
            ...
            for frame in frames:
                ...
                high_freq_ratio = np.sum(high_freq_region) / (
                    np.sum(log_magnitude) + 1e-8
                )
                low_freq_ratio = np.sum(low_freq_region) / (
                    np.sum(log_magnitude) + 1e-8
                )
